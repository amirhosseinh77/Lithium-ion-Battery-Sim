...
class BarDelta_SPKF:
    def __init__(self, BatteryPack, SigmaX0, SigmaW, SigmaV):
        ...
        # initial state
        self.xhat  = np.vstack([ir0, hk0, SOC0, 0]) 
    ...
    def stateEqn(self, current, xnoise=0, oldState=None):
        ...
        Ah = np.exp(-abs(current*self.GParam*self.dt*self.QinvParam_bar/3600))  # hysteresis factor
        ...
        z_k1 = z_k - (self.dt*self.QinvParam_bar/3600)*current
        ib_k1 = ib_k + xnoise[1,:]

        h_k1 = np.clip(h_k1, -1, 1)
        z_k1 = np.clip(z_k1, -0.05, 1.05)

        newState = (iR_k1, h_k1, z_k1, ib_k1)
        return newState
    ...
    def outputEqn(self, current, ynoise=0, state=None):
        ...
        sik = 0
        voltage = self.OCVfromSOC(z_k) + self.MParam*h_k + self.M0Param*sik - self.RParam*iR_k - self.R0Param_bar*(current-ib_k) + ynoise
        return voltage
